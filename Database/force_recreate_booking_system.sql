...
PRINT '=========================================='
PRINT 'FORCE RECREATE BOOKING SYSTEM'
PRINT '=========================================='
PRINT ''

-- =============================================
-- BƯỚC 1: TÌM VÀ XÓA TẤT CẢ FOREIGN KEY CONSTRAINTS
-- =============================================
PRINT '→ Tìm và xóa tất cả Foreign Key Constraints...'

-- Tìm tất cả foreign key constraints liên quan đến các bảng booking
DECLARE @sql NVARCHAR(MAX) = ''

-- Xóa foreign key constraints từ Booking_Service
...
WHERE OBJECT_NAME(referenced_object_id) IN ('Booking', 'PetService', 'Doctor', 'Staff', 'Customer', 'PET')

-- Xóa foreign key constraints từ Booking
SELECT @sql = @sql + 'ALTER TABLE ' + QUOTENAME(OBJECT_SCHEMA_NAME(parent_object_id)) + '.' + QUOTENAME(OBJECT_NAME(parent_object_id)) + ' DROP CONSTRAINT ' + QUOTENAME(name) + ';' + CHAR(13)
FROM sys.foreign_keys 
WHERE OBJECT_NAME(parent_object_id) IN ('Booking', 'Booking_Service')

-- Thực thi các câu lệnh xóa constraints
IF LEN(@sql) > 0
BEGIN
    PRINT '  → Xóa foreign key constraints...'
    EXEC sp_executesql @sql
    PRINT '  ✓ Đã xóa foreign key constraints'
END
ELSE
BEGIN
    PRINT '  ✓ Không có foreign key constraints cần xóa'
END

PRINT ''

-- =============================================
-- BƯỚC 2: XÓA CÁC BẢNG (KHÔNG CẦN KIỂM TRA TỒN TẠI)
-- =============================================
PRINT '→ Xóa các bảng...'

-- Xóa bảng con trước
IF OBJECT_ID('Booking_Service', 'U') IS NOT NULL
BEGIN
...
-- Xóa bảng PetService
IF OBJECT_ID('PetService', 'U') IS NOT NULL
BEGIN
...
-- =============================================
-- BƯỚC 7: TẠO FOREIGN KEY CONSTRAINTS (SAU KHI TẠO XONG BẢNG)
-- =============================================
PRINT '→ Tạo Foreign Key Constraints...'

-- FK từ Booking đến Customer
...
BEGIN
    ...
    PRINT '  ✓ FK_Booking_Customer created'
END

-- FK từ Booking đến PET
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'PET')
BEGIN
    ALTER TABLE Booking 
    ADD CONSTRAINT FK_Booking_Pet 
    FOREIGN KEY (pet_id) REFERENCES PET(id)
    ON DELETE NO ACTION ON UPDATE NO ACTION
    PRINT '  ✓ FK_Booking_Pet created'
END

-- FK từ Booking đến Doctor
ALTER TABLE Booking 
ADD CONSTRAINT FK_Booking_Doctor 
FOREIGN KEY (doctor_id) REFERENCES Doctor(doctor_id)
ON DELETE SET NULL ON UPDATE NO ACTION
PRINT '  ✓ FK_Booking_Doctor created'

-- FK từ Booking đến Staff (nếu bảng Staff tồn tại)
...
BEGIN
    ...
    ON DELETE SET NULL ON UPDATE NO ACTION
    PRINT '  ✓ FK_Booking_Staff created'
END

-- FK từ Booking_Service đến Booking
ALTER TABLE Booking_Service 
ADD CONSTRAINT FK_BookingService_Booking 
FOREIGN KEY (booking_id) REFERENCES Booking(booking_id)
ON DELETE CASCADE ON UPDATE NO ACTION
PRINT '  ✓ FK_BookingService_Booking created'

-- FK từ Booking_Service đến PetService
ALTER TABLE Booking_Service 
ADD CONSTRAINT FK_BookingService_Service 
FOREIGN KEY (service_id) REFERENCES PetService(service_id)
ON DELETE NO ACTION ON UPDATE NO ACTION
PRINT '  ✓ FK_BookingService_Service created'
...
PRINT ''
PRINT '=========================================='
PRINT '✅ HOÀN THÀNH FORCE RECREATE BOOKING SYSTEM'
PRINT '=========================================='
PRINT ''
