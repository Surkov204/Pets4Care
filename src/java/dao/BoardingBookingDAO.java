...
/**
 * DAO cho BoardingBooking Xử lý các thao tác database cho đặt phòng lưu trú
 *
 * @author ASUS
 */
public class BoardingBookingDAO {
    ...
    public boolean addBoardingBooking(BoardingBooking booking) {
        ...
        try (Connection conn = DBConnection.getConnection(); PreparedStatement pstmt = conn.prepareStatement(INSERT_BOARDING_BOOKING,
    ...
    public List<BoardingBooking> getBoardingBookingsByCustomerId(int customerId) {
        ...
        try (Connection conn = DBConnection.getConnection(); PreparedStatement pstmt = conn.prepareStatement(SELECT_BY_CUSTOMER_ID)) {
    ...
    /**
     * Lấy danh sách boarding bookings theo customer ID, khoảng ngày và phân
     * trang
     */
    ...
    public boolean updateBookingStatus(int bookingId, String status) {
        ...
        try (Connection conn = DBConnection.getConnection(); PreparedStatement pstmt = conn.prepareStatement(UPDATE_STATUS)) {
    ...
    public boolean deleteBoardingBooking(int bookingId) {
        ...
        try (Connection conn = DBConnection.getConnection(); PreparedStatement pstmt = conn.prepareStatement(DELETE_BOOKING)) {
    ...
    public boolean updateStatus(int bookingId, String newStatus) {
        String sql = "UPDATE boarding_bookings SET status = ?, updated_at = SYSDATETIME() WHERE booking_id = ?";
    ...
    public String getRoomType(int bookingId) {
        String sql = "SELECT room_type FROM boarding_bookings WHERE booking_id = ?";
        try (Connection con = DBConnection.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
    ...
    public double getPricePerDay(int bookingId) {
        String sql = "SELECT price_per_day FROM boarding_bookings WHERE booking_id = ?";
        try (Connection con = DBConnection.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setInt(1, bookingId);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    public boolean updateCheckoutAndPrice(int bookingId, Timestamp actualCheckout, double totalPrice) {
        String sql = """
        UPDATE boarding_bookings
        SET check_out_date = ?, total_price = ?, status = N'Đã thanh toán', updated_at = SYSDATETIME()
        WHERE booking_id = ?
    """;
        try (Connection con = DBConnection.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
     ...
     public List<BoardingBooking> getBookings(String status, String keyword) {
        List<BoardingBooking> list = new ArrayList<>();

        StringBuilder sql = new StringBuilder(
            "SELECT b.booking_id, b.customer_id, c.name AS customer_name, " +
            "b.room_type, b.price_per_day, b.boarding_days, b.check_in_date, b.check_out_date, " +
            "b.check_in_time, b.check_out_time, b.pet_info, b.special_notes, " +
            "b.emergency_phone1, b.emergency_phone2, b.status, b.total_price, " +
        ...
        if (status != null && !"all".equalsIgnoreCase(status)) {
            switch (status) {
                case "pending":
                    sql.append("AND b.status = N'Chờ xác nhận' ");
                    break;
                case "waiting":
                    sql.append("AND b.status = N'Chờ nhận' ");
                    break;
                case "using":
                    sql.append("AND b.status = N'Đang sử dụng' ");
                    break;
                case "paid":
                    sql.append("AND b.status = N'Đã thanh toán' ");
                    break;
                case "cancel":
                    sql.append("AND b.status = N'Đã hủy' ");
                    break;
            }
        }

        if (keyword != null && !keyword.trim().isEmpty()) {
            sql.append("AND (c.name LIKE ? OR CAST(b.booking_id AS NVARCHAR) LIKE ?) ");
        }
        ...
        try (Connection con = DBConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                b.setBoardingDays(rs.getInt("boarding_days"));
                b.setCheckInDate(rs.getTimestamp("check_in_date"));
                b.setCheckOutDate(rs.getTimestamp("check_out_date"));
                b.setCheckInTime(rs.getString("check_in_time"));
                b.setCheckOutTime(rs.getString("check_out_time"));
                b.setPetInfo(rs.getString("pet_info"));
                b.setSpecialNotes(rs.getString("special_notes"));
                b.setEmergencyPhone1(rs.getString("emergency_phone1"));
                b.setEmergencyPhone2(rs.getString("emergency_phone2"));
     ...
     // 🔹 Checkout sớm, tính lại tiền dựa vào ngày thực tế

    public boolean checkoutEarly(int bookingId, Timestamp actualCheckoutDate) {
        String sql = "UPDATE boarding_bookings "
                + "SET check_out_date = ?, "
                + "total_price = DATEDIFF(DAY, check_in_date, ?) * price_per_day, "
                + "status = N'Đã thanh toán', updated_at = GETDATE() "
                + "WHERE booking_id = ?";
        try (Connection con = DBConnection.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setTimestamp(1, actualCheckoutDate);
            ps.setTimestamp(2, actualCheckoutDate);
            ps.setInt(3, bookingId);
            return ps.executeUpdate() > 0;
        } catch (SQLException e) {
            e.printStackTrace();
